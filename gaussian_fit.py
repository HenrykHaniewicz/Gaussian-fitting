...
def print_parameters( p0, p1, n ):
    for count, i in enumerate(np.arange( 0, 3*n, 3 )):
        print( f"Gaussian {count + 1}: {p0[i]:.4f} -> {p1[i]:.4f}\t{p0[i + 1]:.4f} -> {p1[i + 1]:.4f}\t{p0[i + 2]:.4f} -> {p1[i + 2]:.4f}" )
        ...
        if is_masked:
            ...
            if verbose:
                if n_gauss == 1:
                    ...
                    suff = "Gaussians: "
                print( f"Chi-sq for {n_gauss} {suff} {chi2:.4f}" )
            c.append( chi2 )
...
Usage: {scriptname} file [max_num_gauss = 3] [guesses]

where [] indicates that the argument is optional.

file:           the ASCII data file
                should have two columns: Index and Y-value
                comments (denoted '#') are skipped over

max_num_gauss:  maximum number of Gaussians to try (default = {NGAUSS})

guesses:        list of guesses (default = {GUESS})
                format: "[[mid_point, FWHM, amplitude], ..., []]"


Type {scriptname} -h or {scriptname} help to bring up this information
        ''')
        sys.exit(1)
    elif len(sys.argv) == 3:
